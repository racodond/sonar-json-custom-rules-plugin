...
public class ForbiddenStringCheck extends SubscriptionVisitorCheck {
  ...
  public List<Tree.Kind> nodesToVisit() {
    return ImmutableList.of(Tree.Kind.KEY, Tree.Kind.STRING);
  }
  ...
  public void visitNode(Tree tree) {
    ...
    if (tree.is(Tree.Kind.KEY)) {
      string = ((KeyTree) tree).actualText();
    } else {
      string = ((StringTree) tree).actualText();
    }

    if (string.toLowerCase().contains(FORBIDDEN_STRING)) {
